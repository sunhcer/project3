...
//                }
    ...
    // 测试当面付2.0生成支付二维码
    public String test_trade_precreate(QROrderRef orderRe) {
        // (必填) 商户网站订单系统中唯一订单号，64个字符以内，只能包含字母、数字、下划线，
        // 需保证商户系统端不能重复，建议通过数据库sequence生成，
        ...
        String subject = orderRe.getCinemaName()+orderRe.getFilmName()+"扫码消费";

        // (必填) 订单总金额，单位为元，不能超过1亿元
        // 如果同时传入了【打折金额】,【不可打折金额】,【订单总金额】三者,则必须满足如下条件:【订单总金额】=【打折金额】+【不可打折金额】
        String totalAmount = orderRe.getOrderPrice()+"";
        ...
        String body = orderRe.getCinemaName()+":购买"+orderRe.getFilmName()+"电影票,扫码消费共计"+orderRe.getOrderPrice()+"元";;

        // 商户操作员编号，添加此参数可以为商户操作员做销售统计
//        pid = 2088102177942562
//#appid = 此处请填写你当面付的APPID
//        appid = 2016093000631014
        String operatorId = "2016093000631014";

        // (必填) 商户门店编号，通过门店号和商家后台可以配置精准到门店的折扣信息，详询支付宝技术支持
        String storeId = "2088102177942562";
        ...
        //得到票的数量
        String[] split = orderRe.getSeatsIds().split(",");
        int quatity=split.length;
        GoodsDetail goods1 = GoodsDetail.newInstance(orderRe.getUUID(), orderRe.getFilmName()+"电影票", (long) orderRe.getOrderPrice(), quatity);
        // 创建好一个商品后添加至商品明细列表
        goodsDetailList.add(goods1);
